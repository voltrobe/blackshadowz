         ...
         <hr>
            ...
            //	include 'connect.php';
                ...
                <div onchange="slidercount('<?php echo $row['id']; ?>',event);" id="<?php echo $row['id']; ?>" rel="absent" data-count="0"  class="make-switch" data-on="success" data-off="warning">
                                <input data-off-text="False" data-on-text="True" name="my-checkbox" class="togglecheck" id='switch-offColor' type="checkbox"  />  </div>                    </div>          		
				</td>
              </tr><?php
                      $i++;  
                    }
...
<script>
  $(function () {
  	var ele= document.querySelectorAll(".make-switch");
  	alert(ele[0].firstNode);
  /*	if(ele[0].childNodes[0].hasClass('switch-on'))
...
<script>
	$('.togglecheck').on('switchChange.bootstrapSwitch', function(event, state) {
...
$('#'+id).attr('data-count',count);
	...
	$.get("processattend.php?id="+id+"&attend="+$('#'+id).attr('rel') ,function(){
